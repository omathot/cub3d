...
/*   Updated: 2024/03/17 16:28:30 by oscar            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    make_floor_ceil(t_param_mlx *param_real)
{
  ...
  while (y < param_real->y_resolution)
  {
    ...
    while (x < param_real->x_resolution)
    {
      ...
      {
        // printf("%u ceil\n", param_real->map.ceil);
        mlx_put_pixel(param_real->image_to_draw_pixel, x, y, get_collor(param_real->map.ceil_rgb->r, param_real->map.ceil_rgb->g, param_real->map.ceil_rgb->b, 255));
        // mlx_put_pixel(param_real->image_to_draw_pixel, x, y, param_real->map.ceil);
      }
      else
      {
        mlx_put_pixel(param_real->image_to_draw_pixel, x, y, get_collor(param_real->map.floor_rgb->r, param_real->map.floor_rgb->g, param_real->map.floor_rgb->b, 255));
        // printf("%u floor\n", param_real->map.floor);
        // mlx_put_pixel(param_real->image_to_draw_pixel, x, y, param_real->map.floor);
      }
      x++;
    }
    y++;
...
void	save_ceil_floor_rgb(t_param_mlx *param)
{
  ...
  printf("Ceil RGB values: R = %d, G = %d, B = %d, temp = %d\n", 
       (param)->map.ceil_rgb->r, 
       (param)->map.ceil_rgb->g, 
       (param)->map.ceil_rgb->b, temp);

printf("Floor RGB values: R = %d, G = %d, B = %d, temp2 = %d\n", 
       (param)->map.floor_rgb->r, 
       (param)->map.floor_rgb->g, 
       (param)->map.floor_rgb->b, temp2);
	// param->map.ceil = get_collor(cr, cb, cg, 255);
	// param->map.floor = get_collor(get_rgb__data(param, 'f', 'r'), get_rgb__data(param, 'f', 'g'), get_rgb__data(param, 'f', 'b'), 255);
  // // printf("ceil is %u, floor is %u\n", param->map.ceil, param->map.floor);
  //   printf("%u ceil return is %u\n", param->map.ceil, get_collor(cr, cb, cg, 255));
  //   printf("%u floor\n", param->map.floor);

  // pause();
  // print_point("deaf", mk_point(cr, cb +cg));
}
