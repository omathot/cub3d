...
t_point *ray_casting_to_find_wall(char **board, t_player player, double angle) {
  ...
  // randiant_angle_looking); // sometimes very very negative values here.
  ...
  t_point furthest_point = mk_point(lenght_check * sin(randiant_angle_looking),
                                    lenght_check * cos(randiant_angle_looking));
  puts("test");
  ...
  // printf("furthwers point fabs %f %f\n", fabs(furthest_point.x),
  // fabs(furthest_point.y));
  if (fabs(furthest_point.x) > fabs(furthest_point.y)) {
    while (furthest_point.x != floor(cur_coord.x)) {
      ...
      if (furthest_point.x >= 0)
        ...
        current_x--;
      cur_coord = mk_point(
          current_x, current_x / tan((direction_ray.angle) * (M_PI / 180)));
      // printf("curentposition looking is (%f, %f)\n", cur_coord.x,
      // cur_coord.y); need to replace with map bound check
      // printf("curentposition looking is (%f, %f)\n", cur_coord.x +
      // player.pos.x, -cur_coord.y + player.pos.y);
      if ((int)round(-cur_coord.y + player.pos.y) < 0 ||
          (int)round(cur_coord.x + player.pos.x) < 0) {
        // printf("curentposition looking is (%f, %f)\n", cur_coord.x,
        // cur_coord.y); puts("found wall at neg values"); return (NULL);
        break;
      }
      if (is_out_of_bound((int)round(cur_coord.x + player.pos.x),
                          (int)round(-cur_coord.y + player.pos.y), board)) {
        // printf("curentposition looking is (%f, %f)\n", cur_coord.x,
        // cur_coord.y); puts("looking beyound map"); return (NULL);
        break;
      }
      if (board[(int)round(-cur_coord.y + player.pos.y)]
               [(int)round(cur_coord.x + player.pos.x)] == '\0') {
        // puts("fazed out bro");
        break;
      }
      if (board[(int)round(-cur_coord.y + player.pos.y)]
               [(int)round(cur_coord.x + player.pos.x)] == '1') {
        // puts("wall!");
        t_point *good_coord = malloc(sizeof(t_point));
        *good_coord = mk_point((int)round(cur_coord.x + player.pos.x),
                               (int)round(-cur_coord.y + player.pos.y));
  ...
  while (furthest_point.y != floor(cur_coord.y) + 1) {
    ...
    if (furthest_point.y >= 0)
      ...
      current_y--;
    cur_coord = mk_point(current_y * tan((direction_ray.angle) * (M_PI / 180)),
                         current_y);
    if ((int)round(-cur_coord.y + player.pos.y) < 0 ||
        (int)round(cur_coord.x + player.pos.x) < 0) {
      // printf("curentposition looking is (%f, %f)\n", cur_coord.x,
      // cur_coord.y); puts("found wall at neg values");
      return (NULL);
    }
    if (is_out_of_bound((int)round(cur_coord.x + player.pos.x),
                        (int)round(-cur_coord.y + player.pos.y), board)) {
      // printf("curentposition looking is (%f, %f)\n", cur_coord.x,
      // cur_coord.y); puts("looking beyound map");
      return (NULL);
    }
    if (board[(int)round(-cur_coord.y + player.pos.y)]
             [(int)round(cur_coord.x + player.pos.x)] == '1') {
...
t_point **view_walls(t_map map) {
  ...
  while (final_angle > current_angle) {
    ...
    // if (current_angle < 0)
    // 	current_wall =  ray_casting_to_find_wall(map, player_position,
    // current_angle + 360); else if (current_angle > 360) 	current_wall =
    // ray_casting_to_find_wall(map, player_position, current_angle - 360); else
    current_wall =
        ray_casting_to_find_wall(map.board, map.player, current_angle);
    ...
    if (current_wall != NULL) {
      if (current_wall_index == 0) {
        ...
        // (*all_walls[current_wall_index]).x,
        // (*all_walls[current_wall_index]).y);
        current_wall_index++;
      } else if ((*current_wall).x == (*all_walls[current_wall_index - 1]).x &&
                 (*current_wall).y == (*all_walls[current_wall_index - 1]).y) {
        // printf("curent (%f, %f), last was (%f, %f)\n", (*current_wall).x
        // ,(*current_wall).y, (*all_walls[current_wall_index - 1]).x,
        // (*all_walls[current_wall_index - 1]).y); puts("dup wall");
      } else {
        all_walls[current_wall_index] = current_wall;
        // printf("wall found at (%f, %f)\n",
        // (*all_walls[current_wall_index]).x,
  ...
  while (all_walls[current_wall_index] != NULL) {
    map_emty[(int)(*all_walls[current_wall_index]).y]
            [(int)(*all_walls[current_wall_index]).x] = '1';
    current_wall_index++;
  }
  print_map(map_emty);
  return (all_walls);
